...
# Basic options
parser.add_argument('--output', type=str, default='/home/hfle/dataset/website_for_replace/ade3_stylized/Artflow_stylized',
...
for index in range(len(content_paths)):
    ...
    with torch.no_grad():
        content = Image.open(str(content_paths[index])).convert('RGB')
        img_transform = test_transform(content, args.size)
        content = img_transform(content)
        content = content.to(device).unsqueeze(0)

        style = Image.open(str(style_paths[index])).convert('RGB')
