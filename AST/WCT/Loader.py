...
class Dataset(data.Dataset):
    def __init__(self,contentPath,stylePath,fineSize):
        ...
        self.cimage_list = sorted([x for x in listdir(contentPath) if is_image_file(x)])
        self.simage_list = sorted([x for x in listdir(stylePath) if is_image_file(x)])
        #self.cimage_list.sort()
        #self.simage_list.sort()
    ...
    def __getitem__(self,index):
        contentImgPath = os.path.join(self.contentPath,self.cimage_list[index])
        styleImgPath = os.path.join(self.stylePath,self.simage_list[index])
        ...
        return contentImg.squeeze(0),styleImg.squeeze(0),self.cimage_list[index], self.simage_list[index]

    def __len__(self):
        # You should change 0 to the total size of your dataset.
        return len(self.cimage_list)

'''
class Dataset(data.Dataset):
    def __init__(self,contentPath,fineSize):
        super(Dataset,self).__init__()
        self.contentPath = contentPath
    ...
    def __getitem__(self,index):
        ...
        # Preprocess Images
        contentImg = transforms.ToTensor()(contentImg)
        return contentImg.squeeze(0),self.image_list[index]
