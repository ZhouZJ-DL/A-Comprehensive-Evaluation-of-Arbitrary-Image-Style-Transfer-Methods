...
parser.add_argument('--contentPath',default='/home/hfle/new_website_source/SANet_WCT/content',help='path to train')
parser.add_argument('--stylePath',default='/home/hfle/new_website_source/SANet_WCT/style',help='path to train')
parser.add_argument('--outf', default='/home/hfle/new_website_source/SANet_WCT/WCT_stylized', help='folder to output images')
...
loader = torch.utils.data.DataLoader(dataset=dataset,
                                     ...
                                     shuffle=False)
'''
ContentDataset = Dataset(args.contentPath,args.fineSize)
ContentLoader = torch.utils.data.DataLoader(dataset=ContentDataset,
                                     batch_size=1,
                                     shuffle=False)
StyleDataset = Dataset(args.stylePath, args.fineSize)
StyleLoader = torch.utils.data.DataLoader(dataset=StyleDataset,
                                     batch_size=1,
                                     shuffle=False)
'''
wct = WCT(args)
def styleTransfer(contentImg,styleImg,imname, sname, csF):
    ...
    vutils.save_image(Im1.data.cpu().float(),os.path.join(args.outf,imname+'_'+sname))
    return
'''
def styleTransfer(contentImg,styleImg,imname, smname, csF):
    ...
    Im1 = wct.d1(csF1)
    # save_image has this wired design to pad images with 4 pixels at default.
    vutils.save_image(Im1.data.cpu().float(),os.path.join(args.outf,os.path.splitext(imname)[0]+'_'+smname))
    return
'''
...
for i,(contentImg,styleImg,imname, sname) in enumerate(loader):
    imname = imname[0]
    sname = sname[0]
    print('Transferring ' + imname + '---' + sname)
    if (args.cuda):
    ...
    # WCT Style Transfer
    styleTransfer(cImg,sImg,imname,sname, csF)
    end_time = time.time()
...
'''
for i,(contentImg,imname) in enumerate(ContentLoader):
    imname = imname[0]
    if (args.cuda):
        contentImg = contentImg.cuda(args.gpu)
    cImg = Variable(contentImg, volatile=True)
    for j,(styleImg, smname) in enumerate(StyleLoader):
        smname = smname[0]
        print('Transferring ' + os.path.splitext(imname)[0] + '_' + smname)
        if (args.cuda):
            styleImg = styleImg.cuda(args.gpu)
        sImg = Variable(styleImg,volatile=True)
        start_time = time.time()
        # WCT Style Transfer
        styleTransfer(cImg,sImg,imname,smname, csF)
        end_time = time.time()
        print('Elapsed time is: %f' % (end_time - start_time))
        avgTime += (end_time - start_time)
